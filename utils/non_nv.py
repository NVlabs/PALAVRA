...
@contextmanager
def temporary_random_numpy_seed(seed):
    """
            ...
            The license for for this function can be found in the root directory under LICENSE_COSMO.
...
def encode_text_with_learnt_tokens(self, text, asterix_token, learnable_codes, base_token = None, is_emb = False):
    ...
    if base_token is None:
        for i in range(len(text)):
            if is_emb:
                learnable_codes.weight[i].retain_grad()
                x_i_longer = torch.cat((x[i][:inds_for_insert[1][i]], learnable_codes.weight[i].unsqueeze(0), x[i][inds_for_insert[1][i]+1:]), 0)
            else:
                x_i_longer = torch.cat((x[i][:inds_for_insert[1][i]], learnable_codes[i], x[i][inds_for_insert[1][i]+1:]), 0)
